...
from shop_agent.tools.custom_tool import QdrantCustomUpsertTool,QdrantDeletionTool
from shop_agent.memory import memory_manager
from shop_agent.server import run_server
    ...
    while True:
        try:
            ...
            if user_data is None:
                ...
                # Instantiate and run the deletion tool
                deletion_tool = QdrantDeletionTool()
                print(f"🧹 Clearing episodic memory for user: {session_user_id}...")
                result = deletion_tool._run(user_id=session_user_id)
                print(f"✅ {result}")
                ...
                break  # <-- This is the essential line that stops the loop.
            ...
            print(f"Please Visit localhost:8000 for the UI")
